...
function BulkLoad() {
  ...
  const selectFile = e => {
    if (e.target.files.length > 0) {
      ...
      if (isExcelFile(file)) {
        ...
        setMessage({
          severity: 'error',
          message: 'Solamente se permiten archivos de excel',
        });
      }
    }
  };
  ...
  const sentAttendeeTemplate = async () => {
    ...
    const results = await genericPostService(
      `${B2C_BASE_URL}/attendee/CreateCollection`,
      excelData,
    );
    console.log(results);
    setLoading(false);
    if (results[1]) {
      setMessage({
        severity: 'error',
        message: 'Se presentó un error cargando plantilla',
      });
      return;
    }
    ...
    <Box>
      <div className={styles.wrapper}>
        ...
        <div className={styles.leftSideContainer}>
          ...
          {excelData.length > 0 ? (
            <button
              className={styles.sendTemplate}
              onClick={() => {
                sentAttendeeTemplate();
              }}
            >
              Enviar Plantilla
            </button>
          ...
          <a
            ...
            href="/template/plantillaAsistentes.xlsx"
            download
          >
            Descargar Plantilla
          </a>
        ...
        <div className={styles.rightSideContainer}>
          {excelData.length > 0 ? (
            ...
            <div className={styles.noTableData}>Vista previa no disponible</div>
          )}
        </div>
        <div>
