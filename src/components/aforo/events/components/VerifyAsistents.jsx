...
export default function VerifyAsistents() {
  ...
  const searchBookings = async () => {
    ...
    const results = await genericGetService(
      `${B2C_BASE_URL}/event/AllBookingsByDocument/${document}`,
    );
    ...
    setMessage({
      message: 'Se ha presentado un error buscando las reservas',
      severity: 'error',
    });
  };
  ...
  const confirmBooking = async bookingId => {
    const results = await genericGetService(
      `${B2C_BASE_URL}/event/updateBooking/${bookingId}/Confirmado`,
    );
    ...
    setMessage({
      message: 'Se ha presentado un error confirmando la reserva',
      severity: 'error',
    });
  };
    ...
    <div className={styles.containerVerifyAsistents}>
      <div className={styles.verifyAsistents}>
        ...
        <div className={styles.entryIdTxtAttendee}>
          ...
          <input
            className={styles.inputDataIdTxtAttendee}
            type="text"
            id="idTxtAttendee"
            value={document}
            onChange={e => {
          ...
          <input
            type="button"
            id="searchBooking"
            style={{ 'border-radius': '6px', 'margin-top': '9px' }}
            value={'Consultar...'}
            onClick={() => {
              searchBookings();
            }}
              ...
              bookings.map((booking, index) => {
                return (
                  <tr>
                    ...
                    <td>
                      {booking.eventId?.date?.split('T')[0]}{' '}
                        ...
                        <input
                          title={
                            isConfirmEnabled(
                              ...
                              ? 'Solo se puede confirmar una hora antes o después del evento'
                              : null
                          }
