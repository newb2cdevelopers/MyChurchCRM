...
export default function Aforo() {
  ...
  const getEvents = async () => {
    ...
    return await genericGetService(
      `${BASE_URL}/event/eventsByChurch/${user.selectedChurchId}`,
      ...
      <div className="aforoMainContainer">
        <div className="toolBarContainer">
          <Stack spacing={2} direction="row">
            <Button variant="contained" style={{ backgroundColor: '#454545' }}>
              Verificar
            </Button>
            <Button
              variant="outlined"
        ...
        {loading ? (
          <BackdropLoader show={loading} message="Consultando los eventos" />
        ) : events.length > 0 ? (
          <BasicTable
